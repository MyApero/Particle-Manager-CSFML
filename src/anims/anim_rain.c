...
static sfRectangleShape *create_shape_start(void)
{
    ...
    sfRectangleShape_setPosition(rect, (v2f){0, -10});
    sfRectangleShape_setSize(rect, (v2f){1200, 10});
...
anim_t *create_anim_rain(void)
{
    ...
    anim_set_scale(anim, (v2f){0.5, 2});
    anim_set_color(anim, sfColor_fromRGBA(60, 70, 100, 130), sfColor_fromRGB(
        0, 0, 0));
    anim_set_shape_start(anim, RECT, create_shape_start());
    anim_set_shape_end(anim, MIRROR, &(v2f){-150, 650});
    return anim;
}
